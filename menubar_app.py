...
class CalorieMenuBarComplete(rumps.App):
    ...
    def refresh_menu(self):
        ...
        # ── LOG ───────────────────────────────────────────────
        self.menu.add(rumps.MenuItem("➕ Log food...", callback=self.show_input))
        self.menu.add(rumps.separator)

        # ── TODAY ─────────────────────────────────────────────
        self.menu.add(rumps.MenuItem("  TODAY", callback=None))
        self.menu.add(rumps.MenuItem(f"  🔥 {cals} / {cal_goal} kcal  ({percentage:.0f}%)", callback=None))
        bar = "█" * int(24 * percentage / 100) + "░" * (24 - int(24 * percentage / 100))
        ...
        self.menu.add(rumps.MenuItem(
            f"  💪 {int(totals['protein'])}g/{p_goal}g   🥑 {int(totals['fat'])}g/{f_goal}g", callback=None))
        self.menu.add(rumps.MenuItem(
            f"  🍞 {int(totals['carbs'])}g/{c_goal}g   🍬 {int(totals['sugar'])}g/{s_goal}g", callback=None))
        self.menu.add(rumps.MenuItem(
            f"  🌿 {int(totals['fiber'])}g/{fi_goal}g   🧂 {int(totals['sodium'])}mg/{na_goal}mg", callback=None))

        weekly_data = self.get_weekly_data()
        avg_week = int(sum(weekly_data['calories']) / 7) if weekly_data['calories'] else 0
        self.menu.add(rumps.MenuItem(f"  7-day avg: {avg_week} kcal/day", callback=None))
        self.menu.add(rumps.separator)

        # ── RECENT MEALS ──────────────────────────────────────
        self.menu.add(rumps.MenuItem("  RECENT MEALS", callback=None))
        logs = summary['log']
        if logs:
            for log in reversed(logs[-5:]):
                ...
                if len(name) > 24:
                    name = name[:21] + "..."
                self.menu.add(rumps.MenuItem(f"  • {name}  {kcal} kcal", callback=None))
            if len(logs) > 5:
                self.menu.add(rumps.MenuItem(f"  + {len(logs) - 5} more today", callback=None))
        else:
    ...
    def show_input(self, _):
        window = rumps.Window(
            message="What did you eat?\n\nTip: Be specific — include portion size, brand, or preparation method.\nExamples: '1 chicken thigh fried', 'KFC original drumstick', '200g salmon grilled'",
            title="🍽️ Quick Log",
            default_text="",
            ok="Log Meal",
            cancel="Cancel",
    ...
    def log_food(self, text):
        try:
            ...
            if result['status'] == 'success':
            ...
            elif result['status'] == 'ai_estimate_pending':
                data = result['data']
                msg = (
                    f"No exact match found. The AI estimated:\n\n"
                    f"  Food:     {data.get('name', text)}\n"
                    f"  Calories: {int(data.get('calories', 0))} kcal\n"
                    f"  Protein:  {int(data.get('protein', 0))}g\n"
                    f"  Fat:      {int(data.get('fat', 0))}g\n"
                    f"  Carbs:    {int(data.get('carbs', 0))}g\n\n"
                    f"Log this?"
                )
                ...
                if confirm.run().clicked:
                    self.tracker.commit_item(data)
                    self.refresh_menu()
                    summary = self.tracker.get_summary()
                    cals = int(summary['totals']['calories'])
                    rumps.notification(
                        title="✅ Logged Successfully",
                        subtitle=data.get('name', text),
                        message=f"Total today: {cals} kcal"
                    )
