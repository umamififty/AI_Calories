...
class FoodDatabase:
    ...
    def fuzzy_search(self, query: str) -> Optional[Dict[str, Any]]:
        ...
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM food WHERE LOWER(name) LIKE LOWER(?) ORDER BY length(name) ASC LIMIT 10",
                (f"%{query}%",)
            )
        ...
        for row in rows:
            candidate = dict(row)
            score = SequenceMatcher(None, q, candidate["name"].lower()).ratio()
            if score > best_score:
                best_score = score
    ...
    def find_candidates(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT * FROM food WHERE LOWER(name) LIKE LOWER(?) ORDER BY length(name) ASC LIMIT ?",
                (f"%{query}%", limit)
            )
            rows = cursor.fetchall()
